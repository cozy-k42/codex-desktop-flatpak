...
.PHONY: help check test build-updater maybe-build-updater update rebuild rebuild-install inspect-upstream inspect-upstream-intel inspect-upstream-intel-devcontainer build-app build-app-fresh setup-native bootstrap-native install-native update-native rebuild-next run-app build-dev-app run-dev-app deb rpm pacman appimage flatpak flatpak-manifest-flathub flatpak-check flatpak-refresh-pins package install service-enable service-status clean-dist clean-state
